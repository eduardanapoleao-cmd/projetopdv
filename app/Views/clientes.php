    ...
    <div class="container">
        ...
        <aside class="sidebar">
            ...
            <nav>
                <ul>
                    ...
                    <li class="active"><a href="<?= BASE_URL ?>/?page=clientes">👥 Clientes</a></li>
                </ul>
            </nav>
        ...
        <main class="main-content">
            ...
            <section class="clientes-form-section">
                ...
                <form method="POST" action="<?= BASE_URL ?>/?page=clientes" class="clientes-form">
                    <div class="campo">
                        <label for="cpf">CPF</label>
                        <input type="text" id="cpf" name="cpf" placeholder="000.000.000-00" maxlength="14"
                            inputmode="numeric" oninput="mascaraCPF(this)"
                            class="<?= !empty($erros) ? 'input-erro' : '' ?>" required>
                    </div>
                    <div class="campo">
                    ...
                    <div class="campo">
                        ...
                        <input type="text" id="telefone" name="telefone" placeholder="(00) 00000-0000"
                            maxlength="15" inputmode="numeric" oninput="mascaraTelefone(this)">
                    </div>
            ...
            <section class="clientes-table-section">
                <h2 class="section-title">
                    ...
                    <span class="badge"><?= count($listaClientes) ?> clientes</span>
                </h2>
                        ...
                        <tbody>
                            ...
                            <?php else: ?>
                                <?php foreach ($listaClientes as $cliente): ?>
                                    <tr>
                                        <td><span class="cpf-badge"><?= htmlspecialchars($cliente['cpf']) ?></span></td>
                                        <td class="nome-cliente"><?= htmlspecialchars($cliente['nome']) ?></td>
                                        <td><?= htmlspecialchars($cliente['endereco']) ?: '<span style="color:#ccc">—</span>' ?></td>
                                        <td><?= htmlspecialchars($cliente['telefone']) ?: '<span style="color:#ccc">—</span>' ?></td>
                                        <td>
                                            <form method="POST" action="<?= BASE_URL ?>/?page=clientes" class="form-total">
                                                <input type="hidden" name="cpf_atualizar" value="<?= htmlspecialchars($cliente['cpf']) ?>">
                                                <input type="number" name="novo_total" value="<?= number_format($cliente['total_compras'], 2, '.', '') ?>" min="0" step="0.01" class="input-total" required>
                                                <button type="submit" name="atualizar_compras" class="btn-salvar-total" title="Salvar total">✔</button>
                                            </form>
                                        </td>
                                        <td>
                                            <form method="POST" action="<?= BASE_URL ?>/?page=clientes" class="form-excluir" onsubmit="return confirm('Excluir o cliente <?= htmlspecialchars($cliente['nome']) ?>?')">
